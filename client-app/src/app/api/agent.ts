...
axios.interceptors.response.use(undefined, error => {
    // İnternet bağlantısı yok ise kullaıcıya bilgilendirme amaçlı
    if (error.message === 'Network Error' && !error.response) {
        toast.error('Network error - make sure API as is running!');
    }
    ...
    if (status === 401 || headers['WWW-Authenticate'].includes('Bearer error="invalid_token", error_description="The token expired at"')) {
        ...
        history.push('/')
        toast.info('Your Session Has Expired, Please Login AGAİN')
    }
    if (status === 400 && config.method === 'get' && data.errors.hasOwnProperty('activityId')) {
        history.push('/notfound');
    }
    if (status === 500) {
        toast.error('Server Error - Check Terminal For More İnfo')
    }
...
const Profiles = {
    ...
    listFollowgins: (username: string, predicate: string) => requests.get(`/profiles/${username}/follow?predicate=${predicate}`),
